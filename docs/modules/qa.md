...
> 状态：历史目标设计，当前未实现。现在的 `ask` 是 Codex app-server 或文字 API 的通用工作区问答，不会构建 Chroma 索引或限定回答必须来自教材。请以 [`../architecture.md`](../architecture.md) 为准。
...
| 检索结果为空 | 直接用问题调用 LLM Provider，告知"未找到相关内容" |
| 问题涉及截图/视觉细节 | 返回对应时间戳链接，提示用户自行查看 |
| 问题超出视频内容 | LLM 在回答中说明，不强制回答 |
