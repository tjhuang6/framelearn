...
工具执行器（预处理阶段）
   ├── yt-dlp 下载视频
   ├── ffmpeg 提取音频（.mp3）→ Whisper 语音转文字（带时间戳）
   └── ffmpeg 提取采样帧
   ↓
规划 Agent        ← 将采样帧（base64）发给 Claude API，分析视频结构，制定转换计划
   ↓
工具执行器（全量阶段）
   ├── ffmpeg 按计划提取关键帧
   └── Tesseract OCR 提取帧内文字
   ↓
内容分析器        ← 将帧（base64）发给 Claude API，筛选关键帧，按时间戳与文字稿对齐
   ↓
文档生成器        ← 将帧+对齐后的文字稿发给 Claude API，整理成教材格式输出
   ↓
