...
2. **决定每张 picture N 的去留**：见下面"## 图片去留 4 态规则"。
3. **新增截图**（可选）：启发式漏了老师提到的关键图，needs_extract=true + 新 timestamp。
...
输入 SRT_MD 长这样（vision API 会按"标记 → 图"的顺序紧邻发给你，marker 里带时间戳方便配对）：
...
![picture 3 @ 53.0s](src/frame_00h00m53s000ms_interval_005.jpg)
```
...
{{
  ...
  "selected_timestamps": [
    ...
    {{"srt_id": 3, "timestamp": 53.0, "needs_extract": false, "source_frame_path": "src/frame_00h00m53s000ms_interval_005.jpg", "reason": "内容匹配，时间点准确"}}
  ]
}}
```
